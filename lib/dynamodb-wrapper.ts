...
export class DynamoDBWrapper {
  ...
  constructor(dynamoDB: any, options?: IDynamoDBWrapperOptions) {
    ...
    this.tableNamePrefix =
      typeof options.tableNamePrefix === "string"
    ...
    this.retryDelayOptions.base = getNonNegativeInteger([
      options.retryDelayOptions.base,
      100,
    ]);
    if (typeof options.retryDelayOptions.customBackoff === "function") {
      this.retryDelayOptions.customBackoff =
        options.retryDelayOptions.customBackoff;
    }
  }
  ...
  ): Promise<DynamoDB.UpdateTableOutput> {
  ...
  ): Promise<DynamoDB.DeleteTableOutput> {
  ...
  public async query(
    ...
    // set default options
    options = options || {};
    options.groupDelayMs = getNonNegativeInteger([
      options.groupDelayMs,
      this.groupDelayMs,
    ]);

    let responses = await this._queryOrScanHelper(
      "query",
      params,
      options.groupDelayMs
    );
  ...
  public async batchWriteItem(
    ...
    options?: IBatchWriteItemOptions
    ...
    for (let tableName of tableNames) {
      ...
      option.partitionStrategy =
        option.partitionStrategy || <string>options.partitionStrategy;
      option.groupDelayMs = getNonNegativeInteger([
        option.groupDelayMs,
        options.groupDelayMs,
        this.groupDelayMs,
      ]);
      option.targetGroupWCU = getPositiveInteger([
        option.targetGroupWCU,
        options.targetGroupWCU,
        5,
      ]);
      option.targetItemCount = getPositiveInteger([
        option.targetItemCount,
    ...
    return _makeBatchWriteItemResponse(
      tableNames,
      responsesPerTable,
      totalRequestItems
    );
  ...
  private async _queryOrScanHelper(
    method: string,
    params: any,
    groupDelayMs: number
  ): Promise<any> {
  ...
  private async _batchWriteItemHelper(
    tableName: string,
    params: DynamoDB.BatchWriteItemInput,
    ...
    const getNextGroup =
      options.partitionStrategy === "EvenlyDistributedGroupWCU"
        ? getNextGroupByTotalWCU
        : getNextGroupByItemCount;
  ...
  private async _callDynamoDB(method: string, params: any): Promise<any> {
    ...
    while (true) {
      ...
      if (shouldRetry && ++retryCount <= this.maxRetries) {
        ...
        let tableName = removePrefix(
          this.tableNamePrefix,
          _extractTableNameFromRequest(params)
        );
...
function _makeBatchWriteItemResponse(
  tableNames: string[],
  responsesPerTable: DynamoDB.BatchWriteItemOutput[][],
...
function _aggregateConsumedCapacityMultipleFromResponses(
  responses: any[],
  result: any
): void {
  let listCCM = [];
...
function _aggregateConsumedCapacityMultiple(listCCM: any[]): any {
...
function _aggregateConsumedCapacity(
  listCC: DynamoDB.ConsumedCapacity[]
  ...
  if (listCC[0].LocalSecondaryIndexes) {
    totalCC.LocalSecondaryIndexes = _aggregateConsumedCapacityForIndexes(
      listCC,
  ...
  if (listCC[0].GlobalSecondaryIndexes) {
    totalCC.GlobalSecondaryIndexes = _aggregateConsumedCapacityForIndexes(
      listCC,
...
function _aggregateConsumedCapacityForIndexes(
  listCC: DynamoDB.ConsumedCapacity[],
...
function _validateBatchWriteItemParams(
  params: DynamoDB.BatchWriteItemInput
): void {
