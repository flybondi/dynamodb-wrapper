...
function transpileLibTask() {
  ...
  return src("lib/**/*.ts").pipe(tsProject()).pipe(dest("bin"));
}

function transpileTestTask() {
  const tsProject = tsc.createProject("tsconfig.json");
  return src("test/**/*.ts").pipe(tsProject()).pipe(dest("test"));
}
...
function posttestCoverageReportsTask() {
  return src("bin/**/*.spec.js").pipe(istanbul.writeReports());
}

function testTask() {
  return src("bin/**/*spec.js").pipe(
    jasmine({
...
exports.test = series(
  exports.build,
  pretestCoverageInstrumentationTask,
  testTask,
  posttestCoverageReportsTask
);
